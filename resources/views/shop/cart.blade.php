        ...
        <div class="row justify-content-center">
            <div class="col-lg-8 p-0 pr-2 pb-2">
                ...
                <div class="bg-white shadow d-flex pr-4" style="border-bottom-width: 2px; border-radius: 20px;">
                    <a class="navbar-brand m-4" href="{{ route('inicio') }}">
                        @if (isset($logo))
                            <img src="{{ asset('storage') . '/images/logos/' . $logo }}"style="width:80px;" />
                        @else
                            <img src="{{ asset('image/logo.png') }}"style="width:80px;" />
                        @endif
                    </a>
                        ...
                        <div class="row m-0 mt-3 bg-white" style="border-radius:20px; border:2px solid lightgray;">
                            <div class="col-1  d-flex p-0">
                                <div class="card w-100 border-0" style="border-radius:20px;">
                                    ...
                                    <button style="margin:auto; outline: none;"><i
                                            class="fa fa-angle-up btn-edit"></i></button>
                                    <div style="padding:0 auto; text-align:center;"><span>{{ $item->quantity }}</span></div>
                                    <button style="margin:auto; outline: none;"><i
                                            class="fa fa-angle-down btn-edit"></i></button>
                            ...
                            <div class="col-8 p-3">
                                ...
                                <spam>Precio Total: ${{ number_format(\Cart::get($item->id)->getPriceSum(), 0, ',', '.') }}
                                </spam>
                    ...
                    <div>
                        @if (!Auth::check())
                            <form action="{{ route('shop.checkout.login') }}" method="get">
                                {{ csrf_field() }}
                                <input type="submit" id="btn-login" value="Ir a comprar login"
                        ...
                        @else
                            <form action="{{ route('shop.checkout.checkout') }}" method="POST">
                                {{ csrf_field() }}
                                <input type="submit" id="btn-checkout" value="Ir a comprar Checkout"
                                    class="btn btn-block font-weight-bold"
                                    style="color:white; background-color:#19A448; border-color:#19A448;" />
                            </form>
                        @endif
        ...
        <script>
            function cero(ruta) {
                ...
                if ({{ \Cart::getTotal() }} == 0) {
                    ...
                    toastr.remove();
                    toastr.error('Agregue al menos un Producto al Carrito de Compras para poder Continuar con la Compra.',
                        'Carro Vacio!', {
                            timeOut: 3000
                        });
                } else {
