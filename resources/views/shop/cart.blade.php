    ...
    <div class="container p-0">
        <div class="row justify-content-center px-4 py-4">
            <div id="div_productos" class="col-lg-8 p-0 pr-2 pb-2">
                ...
                <div id="carro" class="col-12 p-0">
                    @foreach ($cartCollection as $item)
                        <div id="div{{ $item->id }}" class="row m-0 mt-3 bg-white"
                            ...
                            <div class="col-1  d-flex p-0">
                                <div id="dsr{{ $item->id }}" style="display:block;">
                                    <div class="card w-100 h-100 border-0" style="border-radius:20px;">
                                        <button id="{{ $item->id }}_btns" name="{{ $item->stock }}"
                                            value="{{ $item->quantity }}" style="margin:auto; outline: none;"><i
                                                class="btnsor fa fa-angle-up btn-edit"></i></button>
                                        <div style="padding:0 auto; text-align:center; position:relative;">
                                            <div id="{{ $item->id }}_icon" class="w-100 h-100 p-1" hidden
                                                style="position:absolute;">
                                                <div class="spinner-border spinner-border-sm" style="color:gray"
                                                    role="status">
                                                    <span class="sr-only">Loading...</span>
                                                </div>
                                            </div>
                                            <input type="text" id="input{{ $item->id }}" class="w-100"
                                                style="text-align:center" disabled name="nombre"
                                                value="{{ $item->quantity }}">
                                        </div>
                                        <button id="{{ $item->id }}_btnr" name="{{ $item->stock }}"
                                            style="margin:auto; outline: none;"><i
                                                class="btnsor fa fa-angle-down btn-edit"></i></button>

                                    </div>
                                </div>
                            </div>
                            <div class="p-0 col-2 d-flex align-items-center justify-content-center">
                                <img style="max-height:100px;" src="/image/productos/{{ $item->attributes->image }}"
                                    class="p-1">
                            </div>
                            <div class="col-sm-8 col-7 p-3">
                                <h3 class="font-weight-normal overflow-ellipsis mb-0"
                                    style=" white-space: nowrap; overflow: hidden;">
                                    {{ $item->attributes->slug }}
                                </h3>
                                <h3 class="font-weight-bold" style="font-size: 13px">{{ $item->marca }}</h3>
                                <spam style="font-size: 14px"> Precio Unidad:
                                    ${{ number_format($item->price, 0, ',', '.') }}</spam><br>
                                <spam style="font-size: 14px" id="preciototal{{ $item->id }}">Precio Total:
                                    ${{ number_format(\Cart::get($item->id)->getPriceSum(), 0, ',', '.') }}</spam>
                            ...
                            <div class="col-sm-1 col-2 pl-0 pr-1  d-flex align-items-center justify-content-end">
                                <div class="m-0" style="display:flex; justify-content:center; width:50px; height:50px;">
                                    <button id="{{ $item->id }}" onclick="deleted(this)" class="btn-w w-100"><i
            ...
            <div class="col-lg-4 p-0 pl-2">
                <div class="shadow border mt-4 mb-3 p-5 card">
                    <div class="row mb-4 m-0 p-0">
                        <div class="d-flex justify-content-between">
                            <h2 class="p-0 font-weight-bold mr-5">Resumen</h2>
                    ...
                    <div class="row p-0">
                        ...
                        <div class="col-6">
                            <h5 id="subtotal" class="m-0 float-right">${{ number_format(\Cart::getTotal(), 0, ',', '.') }}
                            </h5>
                        </div>
                    </div>
                    <hr class="mt-3 mb-3">
                    <div class="row pb-4">
                        ...
                        <div class="col-6 ">
                            <h4 id="total" class="m-0 float-right">${{ number_format(\Cart::getTotal(), 0, ',', '.') }}
                            </h4>
                        </div>
                    </div>
                    <div>
                        @if (Auth::check())
                            <form action="{{ route('shop.checkout.checkout') }}" method="POST">
                                {{ csrf_field() }}
                                <div onclick="cero()">
                                    <input type="submit" value="Ir a pagar" class="btn btn-block font-weight-bold"
                                        @if (sizeof($cartCollection) <= 0) disabled @endif
                                        style="color:white; background-color:#19A448; border-color:#19A448;" />
                                </div>
                            </form>
                        @else
        ...
        <script>
                    ...
                    if (parseInt(cant.value) < parseInt(stock)) {
                        axios.post("{{ route('shop.cart.update') }}", {
                            ...
                            .then(function(response) {
                                ...
                                document.getElementById("subtotal").innerHTML = "$" + response.data.total
                                    .toLocaleString('de-DE');
                                document.getElementById("total").innerHTML = "$" + response.data.total.toLocaleString(
                                    'de-DE');
                                document.getElementById("preciototal" + response.data.id).innerHTML =
                                    "Precio Total: $" + response.data.sumatotal.toLocaleString('de-DE');
            ...
            function def() {
                if ({{ \Cart::getTotal() }} == 0) {
                    ...
                    toastr.warning('Agregue al menos un Producto al Carrito de Compras para poder Continuar con la Compra.',
                        'Carro Vacio!', {
                            timeOut: 5000
                        });
                }
            ...
            function cero() {
                if (document.querySelector(".btn-comprar").disabled) {
                    toastr.remove();
                    toastr.error('Agregue al menos un Producto al Carrito para poder Continuar con la Compra.',
                    'Carro Vacio!', {
                        timeOut: 5000
                    });
                }
